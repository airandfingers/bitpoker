...
//default properties
if(!options.properties){options.properties = {'gjorb':1}}

if(_.isString(options.loadingText) || _.isNumber(options.loadingText)){options.properties['data-loading-text'] = options.loadingText}
  else{options.properties['data-loading-text'] = buttonText}
...
*/
    ...
    $(this.image).prop( options.properties)
...
var jQueryTarget = $(event.originalEvent.target)
...
//if focus event we are going to just select the focused text
  ...
  else if(event.type === 'change')                  {validate = true; updateValue = false}
...
if(selectTextBox){jQueryTextBox.one('mouseup', function(e){return false}).select()}
  if(validate){validateCashierDisplayBasedOnUserInput(updateValue)}
...
if(otherAmountValue <  cashierData.min ){
  console.log('disabling thingy')
  $(self.images.cashier.addChips.image).button('loading')
  if(updateValue === true){ otherAmountTextBox.val(cashierData.min)}
}
else{
...
if(autoRebuyValue <  cashierData.table_min){
//disable button
$(self.images.cashier.enableAutoRebuy.image).button('loading')
 if(updateValue === true){  otherAmountTextBox.val(cashierData.table_min)}
}
else{
...
$('#cashierDiv').on('mousedown mouseup keydown keyup', self.events.cashier)
$('#cashierDiv').find("input[type = 'text']").on('focus focusout', self.events.cashier)
