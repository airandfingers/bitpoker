...
this.events.possibleRaiseOrBetAttemptFromBetSize = function(event){
console.log('possibleRaiseOrBetAttemptFromBetSize called')
...
//we will toggle raise and bet abilities on
  if(toggleBoolean === 'true' ){enableRaiseAndBetEvents()  }

//we will disable raise and bet abilities
 else if(toggleBoolean === 'false' ){disableOneRaiseOrBetAttempt()  }
...
//disable click on raise/bet image
  $(self.images.raise.image, self.images.bet.image).one('mousedown', disableOneRaiseOrBetClick)

//disable enter on betsize
$(self.images.betSlider.betSize.image).on('keydown.enterRaisesOrBets', disableEnterOnce)

}//disable one raise or bet attempt, via enter or clicking the raise/bet button

function enableRaiseAndBetEvents (){
  //enable click events
    $(self.images.bet.image, self.images.raise.image).off('mousedown', disableOneRaiseOrBetClick)
  $(self.images.bet.image, self.images.raise.image).off('click.preventOneClick mouseup.restoreClick')


//enable hitting enter
 // $(self.images.betSlider.betSize.image).on('keydown.enterRaisesOrBets')
  $(self.images.betSlider.betSize.image).off('keydown.enterRaisesOrBets', disableEnterOnce)
  $(self.images.betSlider.betSize.image).off('keyup.restoreEnter')
...
this.one('mouseup.restoreClick', function(e){ enableRaiseAndBetEvents()})
...
this.events.betSizeChanged = function(){
  console.log('betsize changed called')
...
$('#betSize').focusout(function(event){ self.events.possibleRaiseOrBetAttemptFromBetSize(event)})
...
$(self.images.betSlider.betSize.image).on('keydown.enterRaisesOrBets', function(e){
 var keycode = (event.keyCode ? event.keyCode : event.which)

  if(keycode !== 13) {return}    //if enter key is pressed
console.log('making bet or raise action due to enter key being pressed inside betSize')
...
$('#betSize').on('keyup.correctRaiseAndBetAmount',function(e){
var keycode = (event.keyCode ? event.keyCode : event.which)
  if(keycode !== 13) {return}    //if enter key is pressed
    console.log('correcting raise and bet amount because enter key was unpressed from inside betsize')
    self.possibleRaiseOrBetAttemptFromBetSize()
}) 
