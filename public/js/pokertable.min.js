...
,position:true
               ...
               // $(this.seats[i].openSeat.text).css('line-height', this.seats[i].openSeat.size.y +'px')
                console.log($(this.seats[i].openSeat.text).css('line-height'))
...
if(self.getCurrentStackSizes()[seatNumber] <= 0){
...
self.setPreactionData('permanent', 'displayMessageType', 'seat' , {seat:seatNumber})
}
...
var removeEvent = function(child){

  if(child instanceof createjs.DisplayObject){
  
//remove all
if(!_.isString(eventType)){child.removeAllEventListeners()}

else{//if removing a specific type of event
  //REMOVE PERIOD
    var periodIndex = eventType.indexOf('.')
    if(periodIndex != -1){var eventType  = eventType.substring(0,periodIndex)}

    //remove specific function
if(_.isFunction(options.function)){child.removeEventListener(eventType, options.function)}
//remove all evnets of type
  else{child.removeEventListener(eventType)}
    }//if eventType is a string
...
//remove events
if(this.image && options.image !== false){removeEvent(this.image)}
  if(this.text && options.text !== false){removeEvent(this.text)}
...
self.setPreactionData('permanent', 'sitting_out', false , {seat:player.seat})
...
self.clearExpirationData('permanent', player.seat) //this will clear everything else about the player
 ...
 stagesToUpdate.push (    self.playerSits(player.seat, player.username, player.chips))
  self.setPreactionData('permanent', 'displayMessageType', 'getting_chips', {seat: player.seat, server:false})
